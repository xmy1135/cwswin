...
import torchvision
class Mlp_C2f(nn.Module):
    def __init__(self, in_features, hidden_features = None, out_features = None, act_layer = nn.GELU, drop = 0.):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.conv1 = C2f(in_features, in_features)
...
class Mlp_decoder(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.ReLU6, drop=0.):
        ...
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        #self.fc1 = PConv(in_features, hidden_features)
        self.fc1 = nn.Conv2d(in_features, hidden_features, 1, 1, 0, bias=True)
        self.act = act_layer()
        #self.fc1 = PConv(hidden_features, out_features)
...
class WF(nn.Module):
    def __init__(self, in_channels=128, decode_channels=128, eps=1e-8):
        ...
        self.pre_conv = OREPA(in_channels, decode_channels, kernel_size=3)
        #self.pre_conv = ConvBN(in_channels, decode_channels, kernel_size=3)

        self.weights = nn.Parameter(torch.ones(2, dtype=torch.float32), requires_grad=True)
        self.eps = eps
        #self.post_conv = ConvBNReLU(decode_channels, decode_channels, kernel_size=3)
        self.post_conv = OREPA(decode_channels, decode_channels, kernel_size=3)
